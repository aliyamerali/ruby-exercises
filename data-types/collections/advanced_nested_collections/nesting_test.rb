...
class NestedTest < MiniTest::Test
  ...
  def test_list_of_dishes_names_for_olive_garden
    ...
    #=======================
    dishes_names = []
    stores[:olive_garden][:dishes].each { |hash|
      hash.each {|key, value|
        if key == :name
          dishes_names.append(value)
        end
      }
    }
    #=======================
...
#    skip
  ...
  #   skip
    ...
    full_menu = {}
    stores.each { |key, value|
      value[:dishes].each { |dish|
        dish.each { |key, value|
          if key == :name
            full_menu[value] = dish
          end
        }
      }
    }
